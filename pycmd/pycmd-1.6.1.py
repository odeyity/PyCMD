...
# CD COMMAND
    if "cd" in command:
        pathLegit = os.path.normpath(filePath + "\\" + command.replace("cd ", ""))

        # Checks if path exists
        if ".." in command == True or os.path.exists(pathLegit) == True:
            if filePath == "":
                listdir = drive
            else:
                listdir = filePath
            ...
            elif ":" in command and "/" in command or "\\" in command:
                wholeCd = command.replace("cd ", "")
                wholeCd.rstrip()
                wholeCd.rstrip()
                wholeCd.rstrip()
                if debug == True:
                    print(wholeCd)
                if os.path.exists(os.path.normpath(command.replace("cd ", ""))) == True:
                    filePath = wholeCd
                else:
                    print("This directory does not exist.")


            elif os.path.exists(os.path.normpath(listdir + str(command).replace("cd ", ""))) == True and ":" not in command and "/" not in command or "\\" not in command:
                if debug == True:
            ...
            if ".." not in command:
                ...
                if ":" not in command and "/" not in command or "\\" not in command and ":" not in command:
...
# LIST DIRECTORY COMMAND
    if "listdir" in command.split(" ", -1)[0]:
        print("-------------------------------------------")
        ...
        try:
            ...
            print("This directory requires administrator permissions to view.")
...
# CHANGE DRIVE COMMAND
    ...
    elif "drive" in command.split(" ", -1)[0]:
        print("-------------------------------------------")
...
# DOCS COMMAND
    if "docs" in command.split(" ", -1)[0] or "ccmds" in command.split(" ", -1)[0]:
        print("---------------------")
        ...
        print("                                                                      |  docs")
        print("     Move a file      |                                               |  move [file] [destination]")
        print(" Move to current dir  |                                               |  movetocur [file]")

# MOVE COMMAND
    if "movetocur" in command.split(" ", -1)[0]:
        if os.path.exists(os.path.normpath(command.split(" ", -1)[1])) == True:
            if filePath == "":
                shutil.move(os.path.normpath(command.split(" ", -1)[1]), drive)
            else:
                shutil.move(os.path.normpath(command.split(" ", -1)[1]), filePath)
        else:
            print("This directory does not exist.")

    if "move" in command.split(" ", -1)[0]:
        if os.path.exists(os.path.normpath(command.split(" ", -1)[1])) == True and os.path.exists(os.path.normpath(command.split(" ", -1)[2])) == True:
            shutil.move(os.path.normpath(command.split(" ", -1)[1]), os.path.normpath(command.split(" ", -1)[2]))
        else:
            print("This directory does not exist.")

# DELETE COMMAND
    if "del" in command.split(" ", -1)[0]:
        if os.path.exists(os.path.normpath(command.split(" ", -1)[1])) == True:
            confirm = input("Are you sure? [y/n]")
            if confirm == "y":
                os.rmdir(os.path.normpath(command.split(" ", -1)[1]))
            else:
                print("Command aborted.")

# CREATE COMMAND
    if "create" in command.split(" ", -1)[0]:
